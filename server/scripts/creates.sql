create table Lugar (id_lugar serial,nombre_lugar varchar (300) not null,tipo_lugar varchar (300) not null,fk_lugar integer,constraint fk_lugar_lugar foreign key (fk_lugar) references Lugar (id_lugar),constraint pk_id_lugar primary key (id_lugar));
...
CREATE TABLE Mineral_Empresa (
        ...
        CONSTRAINT pk_id_min_empr PRIMARY KEY(id_mineral_empresa),
...
create table Yacimiento (id_yacimiento serial,nombre_yacimiento varchar (50) not null,kilometros integer not null,fk_lugar integer not null,constraint pk_id_yacimiento primary key (id_yacimiento),constraint fk_yacimiento_lugar foreign key (fk_lugar) references Lugar (id_lugar));
...
create table Status_Yacimiento (id_status_yacimiento serial,fecha_cambio date not null,fk_yacimiento integer not null,fk_tipo_status integer not null,constraint pk_status_yacimiento primary key (id_status_yacimiento),constraint fk_status_yacimiento_yacimiento  foreign key (fk_yacimiento) references Yacimiento (id_yacimiento),constraint fk_tipo_status_yacimiento foreign key (fk_tipo_status) references Tipo_Status (id_tipo_status)   );
